...
fn run() -> Result<()> {
    ...
    loop {
        ...
        write_frame(&mut stdout, &response).context("write TXT worker response")?;
    }
}
...
fn handle_payload(state: &mut WorkerState, payload: &[u8]) -> Result<Vec<u8>> {
    let request: TxtWorkerRequest =
        serde_json::from_slice(payload).context("decode TXT worker request")?;
    match request {
        TxtWorkerRequest::Probe { bytes } => {
        ...
        TxtWorkerRequest::ExtractPreview { bytes, limit } => {
            let (temp, registry, handle) = capability_fixture(&bytes)?;
            let _temp = temp;
            let adapter = TxtAdapter::new();
            let token = CancellationToken::default();
            ...
            let resource_id = babel_domain::core::ResourceId::new();
        ...
        TxtWorkerRequest::LoadBegin {
            source_hash_hex,
            byte_length,
        } => {
            let source_hash = decode_hash(&source_hash_hex)?;
            let temp = TempDir::new().context("create TXT worker temp dir")?;
            ...
            fs::write(&path, []).context("initialize TXT worker CAS object")?;
            let session_id = state.next_session_id;
            state.next_session_id = state
                .next_session_id
                .checked_add(1)
                .context("TXT worker session id overflow")?;
        ...
        TxtWorkerRequest::LoadChunk {
            ...
            if offset != session.received_bytes {
                bail!(
                    "TXT worker load offset mismatch: expected {}, got {}",
                    session.received_bytes,
                    offset
                );
            }
            let bytes = hex::decode(data_hex).context("decode TXT worker load chunk")?;
            if bytes.len() > max_load_chunk_bytes() {
                bail!("TXT worker load chunk exceeds per-frame limit");
            }
            let next = session
                .received_bytes
                .checked_add(bytes.len() as u64)
                .context("TXT worker load byte count overflow")?;
            if next > session.byte_length {
                bail!("TXT worker load exceeds declared byte length");
            }
            use std::io::Write;
            let mut file = fs::OpenOptions::new()
                .append(true)
                .open(&session.path)
                .context("open TXT worker CAS object for append")?;
            file.write_all(&bytes)
                .context("append TXT worker load chunk")?;
            session.received_bytes = next;
            Ok(serde_json::to_vec(&LoadChunkReply {
                received_bytes: session.received_bytes,
            })?)
        }
        TxtWorkerRequest::LoadFinish { session_id } => {
            let loading = match state.sessions.remove(&session_id) {
                ...
                Some(Session::Ready(session)) => {
                    ...
                    bail!("TXT worker session is already loaded");
                }
                None => bail!("unknown TXT worker session {session_id}"),
            };
            if loading.received_bytes != loading.byte_length {
                bail!(
                    "TXT worker load incomplete: expected {}, got {}",
                    loading.byte_length,
            ...
            if actual != loading.source_hash {
                bail!("TXT worker load hash mismatch");
            }
        ...
        TxtWorkerRequest::ProbeLoaded { session_id } => {
        ...
        TxtWorkerRequest::InventoryPage {
            session_id,
            generation_id,
            cursor,
        } => {
        ...
        TxtWorkerRequest::ExtractPage {
            ...
            let cache = session
                .cached_extract
                .as_ref()
                .context("TXT worker extract cache missing")?;
            let start = decode_page_cursor(cursor.as_ref())?;
            if start > cache.units.len() {
                bail!("invalid TXT worker extract cursor");
            }
...
fn loading_session_mut(state: &mut WorkerState, session_id: u64) -> Result<&mut LoadSession> {
    match state.sessions.get_mut(&session_id) {
        ...
        Some(Session::Ready(_)) => bail!("TXT worker session is already loaded"),
        None => bail!("unknown TXT worker session {session_id}"),
    }
}
...
fn ready_session(state: &WorkerState, session_id: u64) -> Result<&ReadySession> {
    match state.sessions.get(&session_id) {
        ...
        Some(Session::Loading(_)) => bail!("TXT worker session is not loaded"),
        None => bail!("unknown TXT worker session {session_id}"),
    }
}
...
fn decode_page_cursor(cursor: Option<&Cursor>) -> Result<usize> {
    match cursor {
        ...
        Some(_) => bail!("invalid TXT worker page cursor"),
    }
}
...
fn hash_file(path: &std::path::Path) -> Result<[u8; 32]> {
    let mut file = fs::File::open(path).context("open loaded TXT worker object")?;
    let mut hasher = Sha256::new();
    let mut buffer = vec![0_u8; max_load_chunk_bytes()];
    loop {
        let read = io::Read::read(&mut file, &mut buffer).context("hash TXT worker object")?;
        if read == 0 {
            break;
        }
...
fn capability_fixture(
    ...
    if bytes.len() > babel_runtime::ipc::MAX_FRAME_BYTES {
        bail!("TXT worker request exceeds frame-sized preview path");
    }
    let temp = TempDir::new().context("create TXT worker temp dir")?;
    let objects = temp.path().join("objects");
    ...
    let path = objects
        ...
        .join(&encoded[2..]);
    fs::create_dir_all(path.parent().context("CAS path has parent")?)?;
    fs::write(&path, bytes).context("write TXT worker CAS object")?;
