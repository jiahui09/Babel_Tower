...
function DiffPage() {
  ...
  if (snapshot.isPending || item.isPending) return <div className="grid h-full place-items-center text-sm text-[var(--text-muted)]">{t("loading")}</div>;
  if (snapshot.isError || item.isError) return <div className="grid h-full place-items-center text-sm text-[var(--danger)]">{(snapshot.error ?? item.error)?.message}</div>;
