...
TXT、Markdown、EPUB 文本和图片文字都必须映射为同一种翻译工作项。格式差异只保留在结构保护、源定位、资源关联和导出规则中，不能让每种格式产生独立的译文状态或独立的保存协议。

```text
TranslationWorkItem
├── unit_id                  项目内稳定身份
├── source_content           原文或识别结果
├── translation_content      人工译文
├── source_structure         不可破坏结构与占位符
├── format_locator           原始资源中的定位
├── context                  前后文与来源关系
├── resource_associations    图片、区域、章节等关联
├── status                   翻译状态
├── validation_results       可导出性问题
└── navigation_position      最近工作位置
```

- `source_content`、`translation_content` 和 `source_structure` 由核心权威存储；前端编辑器 JSON 只是当前视图。
- 图片文字使用 `SpatialRegion` 定位，文本使用 `TextRange` 或 `StructuralPath` 定位，二者均通过 `unit_id` 进入同一搜索、撤销、保存、校验和导出链路。
- 文字识别结果只填入原文候选；人工修正后的原文与人工译文分别保存，OCR 不得自动写入译文。
- 格式专属的 AST、字体采样、背景修补像素和导出往返数据保存在适配器私有产物中，不扩张 TIR 成为万能文件 AST。

### 统一编辑器外壳

三种观察方式共用同一个 `TranslationWorkbench` 外壳和同一份当前工作项状态：
...
切换观察方式只替换中心呈现组件，不重建项目上下文、保存状态、撤销历史或搜索筛选。切换必须携带当前 `unit_id`；有资源关联时，资源视图同时携带 `resource_id` 和 `region_id`。

### 连续图片资源工作流

资源模式必须支持按来源阅读顺序、章节顺序或当前筛选生成稳定队列，而不是只能逐张打开文件：

- 资源队列显示前后项、完成状态、受阻状态和区域数量；“未完成”和“有问题”是同一队列的筛选条件。
- 当前图片内按 `reading_order` 连续处理文字区域；确认当前区域后可进入下一个区域，再进入下一张图片。
- 前后图片只预加载缩略图和识别元数据；当前图片按需加载高分辨率原图，切换时不重挂载整个工作台。
- 离开当前区域前先完成统一保存协议。保存失败时保留输入、阻止静默切换并给出重试入口。
- 资源队列位置写入 `navigation_position`，关闭后恢复到准确的图片、区域、缩放和筛选条件。
- 键盘语义与长文、单元模式一致：`Tab`/`Shift+Tab` 切换区域，`Alt+Up/Down` 切换工作项，`Ctrl/Cmd+S` 保存，`Ctrl/Cmd+Z` 撤销。

### 跨格式能力边界

| 能力 | TXT | Markdown | EPUB | 图片 |
| --- | --- | --- | --- | --- |
| 人工译文与状态 | 支持 | 支持 | 支持 | 支持 |
| 统一上下文跳转 | 支持 | 支持 | 支持 | 支持 |
| 结构保护 | 换行 | 标签、链接、代码、图片目标 | XHTML、实体、引用 | 区域、坐标、画布尺寸 |
| 连续导航 | 章节/单元 | 章节/单元 | spine/资源 | 阅读顺序/区域 |
| 原格式交付 | 文本文件 | Markdown 文件 | EPUB 包 | 派生图片并回写引用 |

这张矩阵是实现约束：新增格式必须声明如何生成工作项、如何定位结构、如何连续导航以及如何导出，不能另起一套编辑器状态模型。

### 第六阶段验证重点

必须验证以下完整路径，而不是只验证单张图片能否显示：

1. 从 EPUB 章节文本跳转到关联图片区域；
2. 在图片中连续完成多个区域并进入下一张图片；
3. 中途关闭并恢复到同一 `unit_id`、区域、缩放和筛选；
4. 切回长文或单元观察方式后，状态、译文、撤销和问题列表一致；
5. 导出同时使用文本翻译和图片派生图，原始 EPUB 与原始图片均未被覆盖。

本阶段只实现基础图片嵌字闭环和连续资源导航；高级背景重建、复杂图层、透视变换和自动字体识别继续后置。
...
- 第六阶段已完成的规划同步：统一工作项、资源队列、导航恢复、桌面组件、IPC 边界和项目库导入已写入阶段记录；下一步是系统文件选择器与真实桌面交互验证，再进入 OCR 与嵌字。
